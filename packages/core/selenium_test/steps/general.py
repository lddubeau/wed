...
return [window.test_platform, window.test_browser, window.test_version];
    ...
    // We have to load the browsers module individually, so we have
    // to explicitly load it from the standalone hierarchy.
    require(["/build/standalone/lib/wed/browsers"], function (browsers) {
        var match_tests = [];
