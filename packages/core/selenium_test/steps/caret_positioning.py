...
@when("the user clicks to the (?P<direction>right|left) of the "
      r"last (?P<what>.*?) element")
...
@then(r'the caret is in the (?P<order>last|first|second) '
      r'"(?P<what>.*?)" element')
...
@then(r"the (?P<label>end|start) label of the last (?P<what>.*?) "
      r"element is selected")
