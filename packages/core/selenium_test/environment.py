...
def start_server(context):
    ...
    def start():
        ...
        context.server = subprocess.Popen(
            ["node", "../../misc/server.js",
             "--static-map=../../node_modules/:/top-npm",
             "localhost:" + port])
...
def after_scenario(context, scenario):
    ...
    if (typeof require !== "undefined" && require.defined) {
      // We're running RequireJS or a RequireJS drop-in replacement.
      wed_defined = require.defined("wed");
    }
    else if (typeof SystemJS !== "undefined") {
      // We're running SystemJS.
      wed_defined = SystemJS.registry.has(SystemJS.resolveSync("wed"));
    }
    else {
      done({ loadError: "cannot determine whether wed is loaded" });
    }

    if (!wed_defined) {
      done({ terminating: false });
      return;
    }

    require(["wed"], function (wed) {
      done({ terminating: !!wed.onerror.isTerminating() });
    }, function (err) {
